...
async function generateReadme() {
  ...
  if (!Array.isArray(docItems)) {
    console.error(
      `Error: Expected docItems to be an array, but got ${typeof docItems}.`,
    );
    console.error(
      "Please verify the structure of 'deno doc --json mod.ts' output.",
    );
    Deno.exit(1);
  }
      ...
      (match, target, linkText) => {
        ...
        if (!target.startsWith("http://") && !target.startsWith("https://")) {
          ...
          if (filePath) {
            ...
            url =
              `https://github.com/RickCogley/hibana/blob/main/${filePath}#${target}`; // Add #target for direct anchor
          } else {
            // Fallback for internal links not explicitly mapped, link to mod.ts with anchor
            url =
              `https://github.com/RickCogley/hibana/blob/main/mod.ts#${target}`;
          }
        }
  ...
  const moduleDoc = docItems.find((item: any) =>
    item.kind === "moduleDoc" && item.name === ""
  );
  ...
  if (moduleDoc && moduleDoc.jsDoc && moduleDoc.jsDoc.doc) {
    ...
    console.warn(
      "Warning: Could not find main module documentation (kind 'moduleDoc' with empty name).",
    );
    ...
    if (item.name && item.kind !== "moduleDoc" && item.kind !== "module") {
      ...
      if (item.functionDef && item.functionDef.params) {
        ...
        for (const param of item.functionDef.params) {
          ...
          readmeContent += `- \`${param.name}\` ${
            param.tsType ? `: \`${param.tsType.repr}\`` : ""
          } - ${
            param.jsDoc && param.jsDoc.doc
              ? processJSDocText(param.jsDoc.doc)
              : ""
          }\n`;
        }
        readmeContent += "\n";
      ...
      if (
        item.functionDef && item.functionDef.returnType &&
        item.functionDef.returnType.jsDoc &&
        item.functionDef.returnType.jsDoc.doc
      ) {
        readmeContent += `**Returns:** ${
          processJSDocText(item.functionDef.returnType.jsDoc.doc)
        }\n\n`;
      }
